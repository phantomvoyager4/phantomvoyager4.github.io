...
function Menu({ open, onClose, palette}) {
    return (
        <nav className={`fixed top-0 left-0 w-50 h-full z-50 transition-transform duration-300 ${open ? 'translate-x-0' : '-translate-x-full'}`} style={{ backgroundColor: palette.navbarBackground }}>
            <div className="flex">
              <img src={menuclose} alt="Close Menu" className="ml-auto pt-2 pr-2 cursor-pointer" onClick={onClose} style={{ filter: `invert(${palette.icon === "#181818" ? 1 : 0})` }} />
            </div>
            <ul className="font-InriaSerif text-lg pt-2 pl-2 " style={{ color: palette.navbarText }}>
